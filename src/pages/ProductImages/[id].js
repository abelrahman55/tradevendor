...
const ProductImages = () => {
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Typography noWrap sx={{ color: 'text.secondary', fontWeight: 500 }}>
                <img style={{ width: '50px' }} src={row?.image} alt='' />
              </Typography>
            </Box>
          </Box>
        )
  ...
  const getCategories = async () => {
    setDataLoading(true)
    await axios
      .get(`${BASE_URL}products/ProductImages/${query?.id}`)
      .then(res => {
        console.log(res)
  ...
  const handleAddFile = async () => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}products/add_new_image/${query?.id}`, formData)
  ...
  const handleDel = id => {
    setAddLoading(true)
    axios
      .get(BASE_URL + `products/del_prod_image/${rowData?.id}`)
      .then(res => {
        if (res.data.status == 'success') {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Typography variant='h3'>{`Add New Product image`}</Typography>
        </DialogTitle>
        <DialogContent
          sx={{
            pb: theme => `${theme.spacing(5)} !important`,
